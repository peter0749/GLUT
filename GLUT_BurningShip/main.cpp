...
GLfloat *in_set(double re, double img){
       ...
       while( counter<TH_HOLD ){
              nZ = std::complex<double>(std::abs(nZ.real()),-std::abs(nZ.imag()));
              nZ = nZ*nZ + Z;
              ++counter;
              if(std::abs(nZ) > 4.0f){
...
void Paint(void){
     ...
     glEnd();
     glutSwapBuffers();
     sprintf(title,"BurningShip   X:%.2lf,%.2lf Y:%.2lf,%.2lf", (double)minX, (double)maxX,\
...
int main(int argc, char **argv)
{
    ...
    consoleID = glutCreateWindow("BurningShip");
